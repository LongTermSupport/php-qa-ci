...
### 1. auto-continue.py ✅ RECOMMENDED
...
### 2. prevent-destructive-git.py ✅ CRITICAL
...
### 3. discourage-git-stash.py ⚠️ OPTIONAL
...
### 4. block-plan-time-estimates.py ⚠️ OPTIONAL
...
### 5. validate-claude-readme-content.py ⚠️ OPTIONAL
...
### 6. enforce-markdown-organization.py ⚠️ OPTIONAL
...
4. Add `.claude/` to `.gitignore` (if not already present)
...
```bash
# Copy specific hooks
cp vendor/lts/php-qa-ci/.claude/hooks/auto-continue.py .claude/hooks/
cp vendor/lts/php-qa-ci/.claude/hooks/prevent-destructive-git.py .claude/hooks/
...
Test hook:
```bash
python3 .claude/hooks/prevent-destructive-git.py < test-input.json
echo "Exit code: $?"
```
  ...
  "hooks": {
    "PreToolUse": [{
      "hooks": [
        {
          "type": "command",
          "command": ".claude/hooks/prevent-destructive-git.py",
          "timeout": 5
        }
      ]
    }]
...
1. `auto-continue.py` - QoL improvement, no downside
2. `prevent-destructive-git.py` - Critical safety, prevents data loss

### Optional Based on Team Standards

Deploy these based on team preferences:
3. `discourage-git-stash.py` - If team wants to discourage stash
4. `block-plan-time-estimates.py` - If "no time estimates" is a standard
5. `validate-claude-readme-content.py` - If instruction-only docs is a standard
...
|------|----------|---------|------------|
| auto-continue.py | ✅ YES | Reduce confirmations | None |
| prevent-destructive-git.py | ✅ YES | Prevent data loss | None |
| discourage-git-stash.py | ⚠️ OPTIONAL | Discourage git stash | Low (has escape hatch) |
| block-plan-time-estimates.py | ⚠️ OPTIONAL | No time estimates in plans | Low (configurable paths) |
| validate-claude-readme-content.py | ⚠️ OPTIONAL | Instructions only in docs | Medium (may block valid content) |
| enforce-markdown-organization.py | ⚠️ OPTIONAL | Enforce doc organization | Medium (opinionated structure) |

**Recommendation**: Always deploy the first two, evaluate others based on team standards.

**Note on enforce-markdown-organization.py**: This hook enforces directory conventions (CLAUDE/, docs/, untracked/). If blocked incorrectly, raise with a human to review project structure.
