...
final class PhpInvokerTest extends TestCase
{
    ...
    public function withoutXdebugBuildsTheNoXdebugIniOnceAndPassesMemoryLimitAndArgs(): void
    {
        ...
        $result = $invoker->withoutXdebug('/lib/bin/tool', ['--flag', 'value'], '/project', ['FOO' => 'bar']);

        self::assertSame('tool ran', $result->output);
        self::assertTrue($result->succeeded());
        $spec = $runner->lastSpec();
        self::assertSame(
            ['/usr/bin/php8.5', '-n', '-c', $this->varDir->path . '/phpqa-no-xdebug.8.5.10.ini', '-d', 'memory_limit=4G', '-f', '/lib/bin/tool', '--', '--flag', 'value'],
            $spec->command,
        );
        self::assertSame('/project', $spec->cwd);
        ...
        $invoker->withoutXdebug('/lib/bin/tool', [], '/project');
    ...
    public function withXdebugUsesTheFullIniSetAndNoDashN(): void
    {
        ...
        $invoker = new PhpInvoker($runner, 'php', '2G', $this->varDir->path);

        $result = $invoker->withXdebug('/lib/bin/phpunit', ['-c', 'x.xml'], '/project', ['XDEBUG_MODE' => 'coverage'], streamOutput: false);

        self::assertSame(2, $result->exitCode);
        self::assertFalse($result->succeeded());
        self::assertSame(['php', '-d', 'memory_limit=2G', '-f', '/lib/bin/phpunit', '--', '-c', 'x.xml'], $runner->lastSpec()->command);
    ...
    public function versionAndXdebugAreProbedFromTheConfiguredBinary(): void
    {
        ...
        self::assertSame('8.5.10', $invoker->version());
    ...
    {
        $runner  = new FakeProcessRunner()->willSucceed('8.5.10')->willSucceed('');
        $invoker = new PhpInvoker($runner, 'php', '4G', $this->varDir->path);
    ...
    {
        $spec = new ProcessSpecDto(['php', '-r', 'echo 1;', 'plain-arg', '/a/b.c'], '/p');
