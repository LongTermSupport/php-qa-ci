...
| N5  | `bash bin/qa` fails with a bare shell syntax error, so the entrypoint should answer every invocation form. | Owner ruling: not an entry-point defect. `bin/qa` is a PHP file behind a `php` shebang, and that is what it stays — so `bin/qa`, `php bin/qa`, `php vendor/bin/qa` and Composer's generated PHP bin proxy all work, which is the shape consumers actually depend on. Prefixing a PHP entrypoint with `bash` is an agent invocation error, and the fix belongs where the error is made: a hooks-daemon command-shape rule, carried as Task 3.2. A shell shim was built and measured first; it answered `bash`/`sh` at the cost of retiring `php vendor/bin/qa`, and the trade was refused. |
...
- [x] ✅ **Task 1.4**: struck. The `bash bin/qa` candidate is N5: `bin/qa` stays the PHP entrypoint by Owner ruling, and the invocation error is answered by Task 3.2 rather than by reshaping the entrypoint. A test pins the forms that ship.
...
- [x] ✅ **Task 1.9**: D6 follow-up — the summary comes from the column a header names (`Forbids` / `Summary` / `Description` / `What`), falling back to the trailing cell. A project that ends its rows with provenance resolves without reshuffling its table.
...
- [ ] ⬜ **Task 3.2**: N5 — a hooks-daemon command-shape rule blocking `bash bin/qa` and `sh bin/qa`, with a one-line correction naming `bin/qa`.
  ...
  **Not started, and deliberately not a php-qa-ci change.** Prefixing a PHP
  ...
  `vendor/bin/qa` argument; the correction is the same command without the prefix.

  **Sizing note**: small and self-contained — one pattern, one message, one test,
...
- [x] Release-bound consequences are recorded in `CHANGELOG.md`: one breaking change (lock contention now exits 75), plus the `rule-doc` project-index contract and three fixes. No invocation form changes, so nothing a consumer runs today stops working.
